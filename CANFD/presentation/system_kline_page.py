...
class KLineCanvas(FigureCanvasQTAgg):
    ...
    def draw_bars(self, bars):
        ...
        self.count_axes.set_xticklabels(
            [
                datetime.fromtimestamp(self.bars[index].bucket_start).strftime("%H:%M:%S")
    ...
    def _style_axes(self):
        ...
        self.main_axes.set_ylabel(
            f"{self.value_name} ({self.unit})",
            color="#344256",
            fontsize=9,
        )
    ...
    def _on_mouse_move(self, event):
        ...
        self.annotation.set_text(
            f"{timestamp}\n"
            f"首值 {bar.first:.{self.precision}f}  最高 {bar.maximum:.{self.precision}f}\n"
            f"最低 {bar.minimum:.{self.precision}f}  最新 {bar.latest:.{self.precision}f} {self.unit}\n"
    ...
    def set_bars(self, bars):
        ...
        if not bars:
            ...
            self.summary_label.setText(
                ...
                f"周期最低 {value(latest.minimum)}    本周期采样 {latest.sample_count}"
            )
...
class SystemKLinePage(QWidget):
    ...
    def __init__(self, data_store=None, parent=None):
        ...
        self.metric_order = tuple(self.METRICS)
        self.data_store = data_store or TrendDataStore(metrics=self.METRICS.keys())
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        self.period_selector.setToolTip("每根 K 线聚合的采样时间")
        ...
        self.window_selector.setToolTip("图表保留的最近 K 线数量")
    ...
    def bars_for(self, cluster_index, metric, period_seconds=None, max_bars=None):
        if metric not in self.METRICS:
            return []
        period_seconds = period_seconds or self.period_selector.currentData()
        max_bars = max_bars or self.window_selector.currentData()
    ...
    def _active_metric(self):
        ...
        if index < 0 or index >= len(self.metric_order):
            return self.metric_order[0]
        return self.metric_order[index]
    ...
    def _update_status(self, metric=None, bars=None):
        ...
        if bars:
            ...
            self.status_label.setText(
                f"{definition['name']}已采样 {sample_count} 次，显示最近 {len(bars)} 根 K 线，最新周期 {latest_time}。"
            )
        else:
