...
class BatteryMonitorTem(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setWindowTitle('单体电压温度 (单位: 摄氏度)')
        ...
        for i in range(int(config["LECU_NUM"]*int(config["CELL_Tem_NUM"]))):
            label = QLabel(f'{i + 1}')
            self.grid.addWidget(label, i // config["CELL_Tem_NUM"], (i % config["CELL_Tem_NUM"]) * 2)

            lineEdit = QLineEdit()
            lineEdit.setPlaceholderText('0')
            lineEdit.setAlignment(Qt.AlignmentFlag.AlignCenter)
            self.lineEdits.append(lineEdit)
            self.grid.addWidget(lineEdit, i // config["CELL_Tem_NUM"], (i % config["CELL_Tem_NUM"]) * 2 + 1)
        ...
        for i, voltage in enumerate(voltages):
            if i < len(self.lineEdits):  # 确保不会超出范围
                ...
                if voltage == max_voltage:
                    ...
                    self.lineEdits[i].setStyleSheet("background-color: yellow;")
                else:
