...
class HistoryLogPage(QWidget):
    ...
    def _build_ui(self):
        ...
        self.status_edit.setMinimumWidth(300)
        ...
        self.table.setHorizontalHeaderLabels([header for header, _field in HISTORY_LOG_COLUMNS])
    ...
    def append_record(self, record):
        ...
        for column_index, (_header, field_name) in enumerate(HISTORY_LOG_COLUMNS):
    ...
    def save_records_to_csv(self, path):
        with open(path, "w", newline="", encoding="utf-8-sig") as file_obj:
            ...
            writer.writerow([header for header, _field in HISTORY_LOG_COLUMNS])
            for record in self.records:
                writer.writerow(
                    [
                        _format_cell_value(getattr(record, field_name, ""))
                        for _header, field_name in HISTORY_LOG_COLUMNS
                    ]
                )
